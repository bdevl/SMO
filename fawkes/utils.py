...
def ExtractDirichletValues(bcs):
    ...
    dofs = np.array(
        [dof for bc in bcs for dof in bc.get_boundary_values().keys()], dtype=np.int
    )
    vals = np.array(
        [val for bc in bcs for val in bc.get_boundary_values().values()],
        dtype=np.double,
    )
...
def ConstructDiscontinuousGalerkinScalarFunctionSpaceMapping(V):
    ...
    if (
        ...
        or V.ufl_element().degree() != 0
        or V.num_sub_spaces() != 0
    ):
        raise TypeError(
            "This mapping can only be applied to zero order discontinuous scalar Galerkin function spaces"
        )
...
def AssembleMeshOverlapMatrix(Vc, Vf):
    ...
    if meshc.num_cells() > meshf.num_cells():
        raise ValueError("Input is garbage. Coarse mesh has more cells than fine mesh")

    warnings.warn(
        "AssembleMeshOverlapMatrix : Function assumes that Vc and Vf are not conforming function spaces."
    )
    ...
    num_bytes = Vc.dim() * Vf.dim() * 8

    if num_bytes > 100 * (1024 ** 2):
        warnings.warn("Dense allocation of _W exceeds more than 100MB")
    elif num_bytes > 4096 * (1024 ** 2):
        raise Exception("Dense allocation of _W would exceeds 4GB")
...
def AssembleBasisFunctionMatrix(V, points, *, Global_V=None, ReturnType="scipy"):
    ...
    for row, pt in enumerate(points):
        ...
        basis_values = element.evaluate_basis_all(
            pt, vertex_coordinates, cell_orientation
        )
        ...
        if dim > 1:
            ...
            col_indices = dmap.cell_dofs(cell_id)
            mat.setValues(
                row_indices, col_indices, basis_values, PETSc.InsertMode.INSERT_VALUES
            )
